require 'helpers/sitemap_helper'
...
class CustomExtension < Middleman::Extension
  ...
  def manipulate_resource_list(resources)
    ...
    resources.each do |resource|
      if resource.instance_of?(Middleman::Sitemap::Extensions::RedirectResource) or resource.path.include?('.yml') or resource.ignored
      ...
      if File.exist?(data_file) and data_file != resource.file_descriptor.full_path.to_s
        data = DataStoreFile.new(data_file, File.basename(data_file), File.basename(data_file, 'yml'), @@app_data)
