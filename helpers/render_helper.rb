...
module RenderHelper
  ...
  def render_page(page, options)
    if config[:stage] == 'develop'
      page.render({layout: 'raw'}, options)
    else
      @@page_content ||= {}
      @@page_content[page.destination_path] ||= page.render({layout: 'raw'}, options)
      @@page_content[page.destination_path]
    end
  end
  ...
  def snippet(name, force_tabs=false, always_file_tabs=true)
    snippet = nil
    ...
    if snippet
      if language and not force_tabs
        ...
        out << "<div class='snippet-header'>"
          if always_file_tabs || (!always_file_tabs && languages.length > 1)
  ...
  def markdown_without_erb(content)
  ...
  def anchorize(string)
    if string.nil?
      ...
      HTMLEntities.new.decode(string).gsub(/\s/, "-").gsub(/[()\/",`'&<>\.]/, "").downcase
    end
  end
  ...
  def scan_format_link(format, prefix)
    unless format
      return ''
    end

    format = format.gsub(/([^\\])</, '\1&lt;').gsub(/([^\\])>/, '\1&gt;').gsub(/\\([<>])/, '\1')
    format.scan(/\#{(.*?}?)}/).each do |match|
      format.gsub!("\#{#{match[0]}}", "<a href='##{prefix}-#{anchorize(match[0])}'>#{match[0]}</a>")
    end

    format.scan(/\[([^\s\]].*?)\]\((.*?)\)/m).each do |match|
      format.gsub!("[#{match[0]}](#{match[1]})", "<a href='#{match[1]}'>#{match[0]}</a>")
    end

    format.gsub(/(^.*?)(\s\/\/.*?$)/, '\1<span class="comment">\2</span>')
  end

  def methods_for_param(parameter)
    methods = []

    app_data.api_methods.flatten.each do |method|
      if method&.scope
        if parameter.scope.include?(method.scope) and (not method.except or not method.except.include?(parameter.name))
          methods.push(method)
        end
      end

      if method&.only and method.only.include?(parameter.name)
        methods.push(method)
      end
    end
  ...
  def method_link(api_method)
    "/doc/api-reference/api-methods/#{api_method.base_file_name.gsub('_', '-')}/"
  end

  def bold_anchor(text, prefix='')
    "<strong id='#{prefix}#{anchorize(text)}' class='linkable'>#{text}</strong>"
  end
  ...
  def method_toc(method_name)
    ...
    "[#{api_method.name}](#{method_link(api_method)})"
  end
